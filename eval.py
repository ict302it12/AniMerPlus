...
def main(args):
    ...
    aves_evaluator = Evaluator(smal_model=model.aves, image_size=cfg.MODEL.IMAGE_SIZE)
    ...
    if args.dataset.upper() == "ALL":
        for key in cfg_eval_dataset.keys():
            ...
            eval_one_dataset(cfg_eval_dataset[key], default_cfg, cfg, model, 
                             evaluator=aves_evaluator if key in  ["CUB", "CTRLANI3DPLUS", "COWBIRD"] else smal_evaluator, 
                             aug_cfg=aug_cfg, 
                             key=key)
        ...
        eval_one_dataset(cfg_eval_dataset[args.dataset], default_cfg, cfg, model, 
                         evaluator=aves_evaluator if args.dataset in  ["CUB", "CTRLANI3DPLUS", "COWBIRD"] else smal_evaluator, 
                         aug_cfg=aug_cfg, 
                         key=args.dataset)
...
def eval_one_dataset(dataset_cfg, default_cfg, cfg, model, evaluator, aug_cfg, key):
    ...
    for i, batch in enumerate(bar):
        ...
        with torch.no_grad():
            output = model(batch)
            if "aves_output" in output or "smal_output" in output:
                output = output["aves_output"] if "aves_output" in output else output["smal_output"] 

        if key in ["ANIMAL3D", "CONTROL_ANIMAL3D", "CTRLANI3DPLUS"]:
